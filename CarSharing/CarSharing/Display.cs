...
public class Display
{
    ...
    {   // --
        ...
        while (!isFinished)
        {
            ...
            ShowHeader();
            // Überprüft den Wert links ob dieser dem == -1 oder null entspricht. Ja: (linke Seite von dem : wird der Variable zugewiesen) Nein: rechte Seite
            string timeString = (carFilter.StartTime == -1 || carFilter.EndTime == -1)? "-" : GetTimeFromMinutes(carFilter.StartTime) + " - " + GetTimeFromMinutes(carFilter.EndTime);
            string durString = (carFilter.Duration == -1) ? "-" : carFilter.Duration + "minutes";
            string availString = (carFilter.Available == null ) ? "-" : carFilter.Available;
            string engineString = (carFilter.Engine == null ) ? "-" :  carFilter.Engine;
            string priceString = (carFilter.TotalPrice == -1) ? "-" : carFilter.TotalPrice + " \u20AC";

            //-- (?) Berechnet aus der Methode GetTimeFromMinutes die korrekte Zeit und stellt sie dann in der Konsole da
            Console.WriteLine("[time] Time: " + timeString);
            Console.WriteLine("[dur] Duration: " + durString);
            Console.WriteLine("[avail] Show only available cars (yes/no): " + availString);
            Console.WriteLine("[engine] Engine type: " + engineString);
            Console.WriteLine("[price] Total price: " + priceString);
            ...
            if (userInput.ToLower() == "time")
            {
                ...
                // -- int.Parse = wandelt input_startTime in ein int Wert um
    ...
    public void ShowSearchCar(int currentPage = 1)
    {
        ...
        ShowHeader();
        // Ruft aus FileCar die GetCarList() auf und speichert diese in cars 
        ...
        Console.WriteLine(" ");
        // getCarDetails = zeigt auch die Details über das Auto an | Parameter werden hier zugewiesen von GetCarAtIndex()
        ...
        {
            // Ruft auf was in dem ersten Eintrag steht
        ...
        else if (userInput.ToLower() == "n")
        {
            // 
            currentPage += 1;
            // sorgt dafür, dass die aktuelle Seite nicht weiter als vorhandene Seiten geht (!page 4 of 2!)
        ...
        {
            // sorgt dafür, dass die aktuelle Seite nicht niedriger als Seitenanzahl geht
            currentPage -= 1;
            if (currentPage < 1)
            {
                currentPage = 1;
    ...
    // Überprüft ob der inout vom username korrekt ist,  wenn ja -> true nein -> false + error Meldung (parameter givenUsername: soll überprüft werden)
    public bool IsUserNameAcceptable(string givenUsername)
    {
        ...
        // Wenn die Anforderung (Nuchstaben) korrekt ist und der user noch nicht erstellt wurde, returnt er den Wert auf true (hat alles Anforderungen erfüllt)
        ...
        {
            // Bei einer falschen Angabe setzt es den Wert auf false
            Console.WriteLine("Error: Your username " + givenUsername + " is NOT alphanumerical or does already exist");
            return false;
        }
    }
    // Zeigt das Registriefeld an (prüft erst username und dann passwort)
    public void ShowRegisterUser()
    {
        ...
        while (!isUsernameAcceptable)
        {
            ...
            if (IsUserNameAcceptable(userInputUN))
            {                
    ...
    // Zeigt das Login Feld an und überprüft ob es diese Login daten schon gibt
    ...
    // Neues Auto kann hier hinzugefügt werden
    ...
    private string GetConsoleInput()
    {
        ...
        // Liest den input (links), wenn keiner vorhanden gibt er rechts wieder
        return Console.ReadLine() ?? "";
    }
    // Zeigt die kleinere Auflistung der Detials an [1] {...}
    ...
    // Wandelt minunten in eine uhrzeit um
    private string GetTimeFromMinutes(int min)
    {
        ...
        // BEISPIEL: 345 min --> / 60 = 5(,75) h (Kommerzahl wird ignoriert weil int) für minuten: modulo
        // p= 5
        // p%5 = 0
    ...
    // Gibt die Stelle index = die Stelle in dem Car Array 
